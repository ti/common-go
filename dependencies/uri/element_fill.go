// Package uri provide a http uri decoder
package uri
...
// fill populates the fields of the element using the information in the node.
...
func (f filler) setPtr(field reflect.Value, n *node) error {
	if field.IsNil() {
	...
	return f.fill(field.Elem(), n)
}

func (f filler) setStruct(field reflect.Value, n *node) error {
...
func (f filler) setAllType(field reflect.Value, i int, values []string) (err error) {
	...
	default:
		err = fmt.Errorf("unsupported type: %s", field.Type().Elem())
	}
	return err
}
