...
func (c *sayClient) HelloStream(ctx context.Context, in *Request, opts ...grpc.CallOption) (Say_HelloStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Say_ServiceDesc.Streams[0], Say_HelloStream_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &sayHelloStreamClient{stream}
...
func _Say_HelloStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(SayServer).HelloStream(m, &sayHelloStreamServer{stream})
}
